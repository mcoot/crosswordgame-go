...
func (s *CrosswordGameE2ESuite) Test_FullGame2x2() {
	...
	_, err = s.client.SubmitPlacement(gameId, 0, 0, 0)
	s.Error(err)

	// Player 0 announces a letter
	submitAnnouncement(s.T(), s.client, gameId, 0, "a")

	// Validate game state after announcement
	gameState = getGameState(s.T(), s.client, gameId)
	s.Equal(types.StatusAwaitingPlacement, gameState.Status)
	s.Equal(1, gameState.CurrentAnnouncingPlayer)
	s.Equal(0, gameState.SquaresFilled)

	// Both players place letters
	submitPlacement(s.T(), s.client, gameId, 0, 0, 0)
	submitPlacement(s.T(), s.client, gameId, 1, 1, 1)
	...
	playerState0 := getPlayerState(s.T(), s.client, gameId, 0)
	s.Equal([][]string{{"A", ""}, {"", ""}}, playerState0.Board)
	playerState1 := getPlayerState(s.T(), s.client, gameId, 1)
	...
	// Both players place letters
	submitPlacement(s.T(), s.client, gameId, 0, 1, 0)
	submitPlacement(s.T(), s.client, gameId, 1, 1, 0)
	...
	submitAnnouncement(s.T(), s.client, gameId, 0, "t")
	submitPlacement(s.T(), s.client, gameId, 0, 0, 1)
	submitPlacement(s.T(), s.client, gameId, 1, 0, 1)
	submitAnnouncement(s.T(), s.client, gameId, 1, "e")
	submitPlacement(s.T(), s.client, gameId, 0, 1, 1)
	submitPlacement(s.T(), s.client, gameId, 1, 0, 0)
